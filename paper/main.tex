...
SMOTE-Tomek significantly expanded the Brute Force training representation, successfully managing the severe imbalance ratio. Consequently, the Stacking Ensemble combined with SMOTE-Tomek was selected as the primary model for its superior balance between maintaining high macro F1-scores and keeping a low False Positive Rate (FPR). On the unseen test set, the final model achieved 99.93\% macro accuracy, 86.10\% macro precision, 90.22\% macro recall, and 86.93\% macro F1-score. Specifically for Brute Force traffic, it delivered 98.70\% precision, 92.66\% recall, 95.58\% F1-score, and a remarkably low 0.0253\% FPR. 

Despite these positive results, the study has limitations. The evaluation relied solely on the CICIDS2017 dataset, and the pipeline was less effective on extremely minority classes like SQL-Injection and Infiltrations. Future research should evaluate the proposed pipeline on newer datasets, implement feature scaling for distance-sensitive models, and explore few-shot learning techniques to reduce reliance on synthetic data for rare attacks. Ultimately, this study demonstrates that effective minority-attack detection requires a holistic approach that concurrently addresses class representation, feature relevance, and model diversity.
