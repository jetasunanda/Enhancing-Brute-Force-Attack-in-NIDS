...
Comparing the ensemble strategies, the Stacking Ensemble provided the highest macro F1-score and better false-positive control compared to the Soft Voting approach. During validation, the Stacking Ensemble achieved 99.93\% macro accuracy, 86.35\% macro precision, 87.20\% macro recall, 84.78\% macro F1-score, 0.0406\% macro FPR, and 98.90\% macro ROC-AUC. Thus, the Stacking Ensemble combined with SMOTE-Tomek was selected as the primary model. Furthermore, when evaluated on the unseen test dataset, the chosen model demonstrated excellent generalization by achieving 99.93\% macro accuracy, 86.10\% macro precision, 90.22\% macro recall, 86.93\% macro F1-score, 0.0364\% macro FPR, and 97.98\% macro ROC-AUC. Specifically for the target Brute Force class, the model reached a 95.58\% F1-score, 98.70\% precision, 92.66\% recall, and 0.0253\% FPR on the test set, confirming the effectiveness of the proposed pipeline.
